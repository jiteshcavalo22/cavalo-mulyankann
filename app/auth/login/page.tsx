...
export default function LoginPage() {
  ...
  const handleSendOtp = () => {
    if (mobile.length === 10) { setStep("otp"); startTimer(); }
  };
    ...
    <div className="min-h-screen bg-gray-50 flex items-center justify-center p-4">
      <div className="w-full max-w-md">
        ...
        <div className="bg-white rounded-lg shadow-lg overflow-hidden border border-gray-200">
          {/* Header — Cavalo style: white bg, yellow accent, navy text */}
          <div className="border-b-4 border-cavalo-yellow p-6 text-center">
            ...
            <h1 className="text-navy text-xl font-bold mt-3">Welcome Back</h1>
              ...
              <div className="space-y-4">
                <div>
                  ...
                  <div className="flex items-center gap-2 mt-1">
                    ...
                    <Input type="tel" value={mobile} onChange={(e) => setMobile(e.target.value.replace(/\D/g, "").slice(0, 10))} placeholder="9876543210" className="flex-1 focus-visible:ring-cavalo-yellow" autoFocus />
                  </div>
                </div>
                <button onClick={handleSendOtp} disabled={mobile.length !== 10} className="btn-cavalo w-full py-2.5 text-sm disabled:opacity-50 disabled:cursor-not-allowed inline-flex items-center justify-center gap-2">
                  Send OTP <ArrowRight className="w-4 h-4" />
                </button>
                <div className="text-center text-sm text-gray-500">
                  Don&apos;t have an account?{" "}
                  <button onClick={() => router.push("/auth/register")} className="text-cavalo-yellow font-semibold hover:underline">Register</button>
                </div>
              </div>
            ) : (
              <div className="space-y-4">
                <div>
                  <div className="flex items-center justify-between mb-3">
                    ...
                    <button onClick={() => setStep("mobile")} className="text-cavalo-yellow text-sm font-semibold hover:underline">Change number</button>
                  </div>
                ...
                <button onClick={handleVerify} disabled={otp.join("").length !== 6} className="btn-cavalo w-full py-2.5 text-sm disabled:opacity-50 disabled:cursor-not-allowed">Sign In</button>
              </div>
            )}
          </div>
        </div>
